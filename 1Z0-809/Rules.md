...
* Stream API
  ...
  * `Stream`
    * There is NO `toMap()` method in stream, instead there is `Collectors.toMap`, which will throw `IllegalStateException` when collected map already contains given key; if duplicates are expected use another version of this method: `Collectors.toMap(b->b.getTitle(), b->b.getPrice(), (v1, v2)->v1+v2`
    * There is NO `reset()` method on stream
    * sorting
